...
vec4 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
   ...
   vec4 diff_color = texture( material.diffuseTexture, fs_in_TexCoords );
...
void main(void) 
{  
    vec3 normal = texture(material.normalTexture, fs_in_TexCoords).rgb;
    normal = normalize(normal * 2.0 - 1.0);
    normal = normalize(fs_in_TBN * normal);
    //vec3 normal  = normalize(fs_in.Normal);
    vec3 viewDir = normalize(viewPos - fs_in_FragPos);

    color = CalcPointLight(light, normal, fs_in_FragPos, viewDir);
