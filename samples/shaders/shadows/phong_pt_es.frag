...
void main()
{  
  vec3 norm = normalize(fs_in_Normal);
  vec3 viewDir = normalize(viewPos - fs_in_FragPos);
  vec4 result = vec4(0, 0, 0, 0);
  float shadow = ShadowCalculation(fs_in_FragPos);

  for(int k = 0; k < int(point_count); k++){
     result += CalcPointLight(pointLights[k], norm, fs_in_FragPos, viewDir, shadow);}
