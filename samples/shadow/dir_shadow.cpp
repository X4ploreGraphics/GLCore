...
int main(int argc, char const *argv[])
{
    ...
    auto height_image = (argc > 1)? utils::image::read_image(argv[1]) : gen_checker_R8({300, 300}, {50, 50});
...
glcore::load_context(glfwGetProcAddress);
#endif
    {
        ...
        auto phong_shader = glcore::Shader{ vert_header + read_file("shaders/shadows/phong_dir.vert"), frag_header + read_file("shaders/shadows/phong_dir.frag") };
        ...
        //auto copy_depth_texture = glcore::DepthStencilTexture{utils::ImageMetaData{ {f_p_dim[0], f_p_dim[1]}, utils::pixel::GREY_STENCIL_24_8::value }, glcore::texture::target::texture_2D, false};
        auto copy_depth_texture = glcore::DepthTexture{utils::ImageMetaData{ {f_p_dim[0], f_p_dim[1]}, utils::pixel::GREY16::value }, glcore::texture::target::texture_2D, false};
        ...
        dir_light.ambient = {0.2, 0.2, 0.2, 1.0};
        dir_light.diffuse = {0.7, 0.7, 0.7, 1.0};
        dir_light.specular = {1, 1, 1, 1};
        dir_light.direction = to_vec3f( glm::normalize( glm::vec3{0, 0, 0} - dir_light_pos ) );
        ...
        auto dir_light_proj = glm::ortho(-5.f, 5.f, -5.f, 5.f, 0.1f, 10.f);
        auto dir_light_view_mat = glm::lookAt(dir_light_pos, glm::vec3{0, 0, 0}, glm::vec3{0, 1, 0});
        ...
        {   
            ...
            auto light_space = dir_light_proj * dir_light_view_mat;
            ...
            auto content_render = [&](){
                ...
                phong_shader.set_uniform("dir_count", utils::vec1f{1});
                phong_shader.set_uniform("dirLights[0].direction", dir_light.direction);
                phong_shader.set_uniform("dirLights[0].ambient"  , dir_light.ambient);
                phong_shader.set_uniform("dirLights[0].diffuse"  , dir_light.diffuse);
                phong_shader.set_uniform("dirLights[0].specular" , dir_light.specular);
